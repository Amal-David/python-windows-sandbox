...
class PathValidator:
    ...
    def validate_guest_path(cls, path: Union[str, Path]) -> Path:
        ...
        # Ensure it's an absolute Windows path
        if not path.is_absolute():
            raise SecurityError("Guest path must be absolute")
...
class CommandValidator:
    ...
    DANGEROUS_COMMANDS = {
        'cmd', 'powershell', 'wmic', 'net', 'netsh', 'reg', 'regedit',
        'schtasks', 'sc', 'shutdown', 'restart', 'format', 'diskpart',
        'del', 'rmdir', 'rd', 'erase', 'attrib', 'cacls', 'icacls'
    }
